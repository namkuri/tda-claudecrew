...
- **구현 위치**: `lib.rs`(팀 생성/모니터링), `ui`(팀 진행 시각화 — 전문가 카드별 상태).
- **작업**: 팀 생성 프롬프트 + `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` 활용, 팀원별 상태/메시지 표시.
- **완료 기준**: 한 작업에 3~5 전문가가 병렬로 보이고 진행됨.
- **주의**: 실험적 기능 제약(세션 재개 등) 고려. 동시성 한도/비용 가드와 연계.

### [ ] V1.2 미리보기(포트)
- **목표**: 에이전트가 띄운 dev 서버를 클릭 한 번으로 브라우저에서 확인.
- **구현 위치**: `lib.rs`(포트 감지), `ui`(미리보기 버튼 + opener로 열기).
- **완료 기준**: 실행 중 서버 포트가 카드에 뜨고 클릭 시 열림.

### [ ] V1.3 작업 공간 영속화/복원
- **목표**: 앱 재시작 후에도 기존 작업 공간 표시.
- **구현 위치**: `lib.rs`(`git worktree list` 파싱 또는 상태 파일), 시작 시 `list_agents`로 복원.
- **완료 기준**: 재시작 후 진행 중/완료 작업이 보드에 복원.
...
## 마일스톤 v2 — "고도화"
### [ ] V2.1 레시피 마켓/공유  ### [ ] V2.2 LSP·정밀편집(MCP)  ### [ ] V2.3 API 자동 전환(헤비 시)
